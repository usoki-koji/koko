...
class ItemConsumeSkillModifier(
    val target: String,
    val filter: AbstractSkillModifierFilter
) : AbstractSkillModifier() {
    override val type: String = Paths.DefaultModifiers.PLAYER_CONSUME
    ...
    override fun apply(applier: SkillsHandler.SkillModifierApplier, player: Player) {
        ...
        if (group != null) {
            for (listedTarget in group.content) {
                PlayerEventHandler.addBlockedItem(
                    player.uuid, MainHelper.safeParseResource(listedTarget), PlayerEventHandler.PlayerBlockScope.CONSUME
                )
            }
        ...
        PlayerEventHandler.addBlockedItem(
            player.uuid, MainHelper.safeParseResource(target), PlayerEventHandler.PlayerBlockScope.CONSUME
        )
    }
    ...
    companion object {
        val CODEC: MapCodec<ItemConsumeSkillModifier> = RecordCodecBuilder.mapCodec { instance ->
            instance.group(
                Codec.STRING.fieldOf("item").forGetter(ItemConsumeSkillModifier::target),
                AbstractSkillModifierFilter.CODEC.fieldOf("filter").forGetter(ItemConsumeSkillModifier::filter)
            ).apply(instance, ::ItemConsumeSkillModifier)
        }
    }
}